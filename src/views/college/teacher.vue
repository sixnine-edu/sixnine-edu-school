...
<template>
  <div class="app-content">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>教师栏</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="columns-top-block" v-if="columnTopList.length>0">
      <div class="column-top-left">
        <div class="panel-block" v-for="(item,index) in columnTopList" :key="index">
          ...
          <div class="content">
            <div class="live-item" :class="{active:curSelect==`${item.columnId}_${citem.liveId}`}"
              v-for="(citem,cindex) in item.liveList" :key="cindex"
              @mouseenter="onMouseEnter($event,item.columnId,citem)" @mouseleave="onMouseLeave"
              @click="toLiveDetail(citem.liveId)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCollegeColumn() {
        this.$http({
          url: this.$http.adornUrl(`/corp/college/collegecolumn/2/0`),
          method: "get",
      ...
      getCollegeTopColumn() {
        this.$http({
          url: this.$http.adornUrl(`/corp/college/collegecolumn/2/1`),
          method: "get",
          params: this.$http.adornParams()
        }).then(({
          data
