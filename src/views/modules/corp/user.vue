    ...
    <avue-crud ref="crud"
      ...
      <template slot="menuLeft">
        <el-button type="primary"
                   icon="el-icon-plus"
                   size="small"
                   v-if="isAuth('corp:user:save')"
                   @click.stop="addOrUpdateHandle()"> {{$t('sysManagement.add')}}</el-button>

        <el-button type="danger"
                   @click="deleteHandle()"
                   v-if="isAuth('corp:user:delete')"
                   size="small"
                   :disabled="dataListSelections.length <= 0"> {{$t('corp.batchDelete')}}</el-button>
      </template>
      <template slot-scope="scope"
                slot="menu">
        <el-button type="text"
                   size="small"
                   v-if="isAuth('corp:user:update')"
                   @click.stop="addOrUpdateHandle(scope.row.userId)"> {{$t('coupon.edit')}}</el-button>

        <el-button type="text"
                   size="small"
                   v-if="isAuth('corp:user:delete')"
                   @click.stop="deleteHandle(scope.row.userId)"> {{$t('coupon.delete')}}</el-button>
      </template>
    </avue-crud>
    <!-- 弹窗, 新增 / 修改 -->
...
<script>
import { tableOption } from '@/crud/corp/user'
