...
<template>
  <div class="photo-preview">
    ...
    <div class="content" ref="content" @scroll="onContenScroll">
      ...
      <div class="pagination">{{title}} <span>{{current+1}}/{{picList.length}}</span></div>
    </div>
      ...
      onContenScroll(e) {
        ...
        for (let i = $imgs.length - 1; i >= 0; i--) {
         if ($imgs[i].offsetTop <= scrollTop) {
           ...
           this.current = i;
           this.$refs['thumb'].scrollTo(0, this.thumbImgs[i].offsetTop - clientHeight/2)
           break;
         }
        }
      }
